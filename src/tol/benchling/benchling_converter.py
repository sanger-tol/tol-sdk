...
class BenchlingConverter(Converter[BenchlingReturn, DataObject]):
    ...
    def convert(self, input_: BechlingGet) -> DataObject:
        return self.__ds.data_object_factory(
            'tissue',  # TODO how to know reliably what the `type` of object is???
            id_=input_.id,
...
class DataObjectConverter(Converter[DataObject, BenchlingWrite]):
    ...
    def __convert_entity_fields(
        ...
        mapped_dict = {
            self.__get_entity_name(object_type, name): {'value': value}
            for name, value in data_dict.items()
        }

        return fields(mapped_dict)
