...
10) pbprep_category: [character]
11) pbprep_shear_type:  [character]
12) pbprep_difficult_shear: [character]
13) pbprep_trouble_shooting: [character]
14) amount_of_dna_needed_ng: [double]
15) pbprep_bead_type: [character]
16) pbprep_spri_type: [character]
17) pbprep_qc_passfail: [character] outcome of QC assessment during decision making.
18) pbprep_next_steps: [character] decision taken after preparation. 
19) pbprep_nanodrop_ngul: [double] nanodrop concentration in ng/ul.
20) pbprep_260_280_ratio: [double] nanodrop 260/280 ratio.
21) pbprep_260_230_ratio: [double] nanodrop 260/230 ratio.
22) pbprep_qubit_ngul: [double] qubit concentration in ng/ul.
23) pbprep_yield: [double] yield.
24) pbprep_femto_avg_frag_size: [double] Femto average fragment size.
25) pbprep_femto_mode_frag_size: [double] Femto mode fragment size.
26) pbprep_gqn: [double] GQN index.
27) pbrep_femto_description: [jsonb] Qualitative description of femto profiles.
28) pbprep_bnt_id: [character] Batches and Tracking legacy id.

NOTES: 

1) This query follows only Benchling Data Model version 2: Results attached to the entity.
2) DISTINCT ON and ORDER BY are applied together to delete duplicated rows present in BWH.
...
SELECT DISTINCT ON (subsam.name$, con.barcode)
	...
	pbgdna.category AS pbprep_category,
	COALESCE(shear_v2.type_of_shearing, shear_v1.type_of_shearing) AS pbprep_shear_type, 
	COALESCE(shear_v2.difficult_shear, shear_v1.difficult_shear) AS pbprep_difficult_shear, 
	COALESCE(shear_v2.trouble_shooting, shear_v1.trouble_shooting) AS pbprep_trouble_shooting,
	COALESCE(shear_v2.amount_of_dna_needed_ng, shear_v1.amount_of_dna_needed_ng) AS amount_of_dna_needed_ng,
	spri.bead_type AS pbprep_bead_type,
	spri.spri_type AS pbprep_spri_type,
	pbdc.passfail AS pbprep_qc_passfail,
	pbdc.next_steps AS pbprep_next_steps,
	nanod.nanodrop_concentration_ngul AS pbprep_nanodrp_ngul,
	nanod._260_280_ratio AS pbprep_260_280_ratio,
	nanod._260_230_ratio AS pbprep_260_230_ratio,
	qbit.qubit_concentration_ngul AS pbprep_qubit_ngul,
	pbyield.yield AS pbprep_yield,
	femtopb.average_fragment_size AS pbprep_femto_avg_frag_size,
	femtopb.mode_fragment_size AS pbprep_femto_mode_frag_size,
	femtopb.gqn AS pbprep_gqn,
	femtopb.femto_profile_description AS pbprep_femto_description, 
