...
class StsSampleToCasmBenchlingConverterFactory:
    ...
    def __init__(self, destination_object_type: str, previous_object_type: str = "", previous_objects: list = None):
        ...
        if previous_object_type and previous_objects:
            for previous_object in previous_objects:
                identifier = self.BENCHLING_OBJECT_MAP[previous_object_type]['primary_attribute']
                key = getattr(previous_object, identifier)
                self.BENCHLING_OBJECT_MAP[previous_object_type]['stored_values'][key] = previous_object
    ...
    def get_converter_class(self) -> DataObjectToDataObjectOrUpdateConverter:
        ...
        class StsSampleToCasmBenchlingConverter(DataObjectToDataObjectOrUpdateConverter):
            def convert(self, data_object: DataObject) -> Iterable[DataObject]:
                sample = data_object
                primary_attr_id = factory.BENCHLING_OBJECT_MAP[factory.destination_object_type]['primary_attribute']
                primary_attr = factory.BENCHLING_OBJECT_MAP[factory.destination_object_type]['attribute_map'][primary_attr_id]
                primary_attr_value = sample.id if 'id' == primary_attr else sample.attributes.get(primary_attr)

                if not self._does_object_exist(primary_attr_value, primary_attr_id):
                    self._populate_relationships(sample)

                    attribute_map = factory.BENCHLING_OBJECT_MAP[factory.destination_object_type]['attribute_map']
                    object_attributes = {
                        key: (sample.id if 'id' == attr_mapping else sample.attributes.get(attr_mapping))
                        for key, attr_mapping in attribute_map.items()
                    }

                    self._sanitize_attributes(object_attributes)
                    factory.BENCHLING_OBJECT_MAP[factory.destination_object_type]['converted_value_identifiers'] = \
                        factory.BENCHLING_OBJECT_MAP[factory.destination_object_type]['converted_value_identifiers'] + [primary_attr]
            ...
            def _populate_benchling_relationships(self, sample):
                """
                    This method populates the attributes with the values from the benchling_relationships.
                    ...
                    Raises:
                        ...
                        This is mainly because the relationships identified within the benchling_relationships are almost always required.
                """
                object_type = factory.destination_object_type
                relationships = factory.BENCHLING_OBJECT_MAP[object_type]['benchling_relationships']
                ...
                for relationship_key in relationships:
                    ...
                    if 'relationship' == relationship['sts_type'] and hasattr(sample, identifier):
                        try:
                            ...
                            search_value = getattr(relationship_obj, relationship['sts_identifier'], None)
                        except StopIteration:
                            continue
                    elif 'field' == relationship['sts_type']:
                        search_value = sample.id if 'sts_id' == relationship['sts_identifier'] else getattr(sample, relationship['sts_identifier'], None)
                    ...
                    if search_value is not None:
                        ...
                        if search_value not in stored_values:
                            ...
                            schema_filter.and_ = {
                                relationship['benchling_search_identifier']: {'eq': {'value': search_value}},
                            }
                            filter_obj = DataSourceFilter()
                            filter_obj.and_ = {'schema_fields': schema_filter}

                            try:
                                benchling_item = next(iter(factory.benchling.get_list(relationship['benchling_object_name'], filter_obj)))
                                stored_values[search_value] = benchling_item.id
                            except StopIteration:
                                print('search_value')
                                print(search_value)
                                raise Exception(
                                    f'Sample not ready for import: Sample #{sample.id} is missing the relationship for {relationship_key}'
                                )

                        sample.attributes[relationship_key] = stored_values[search_value]
                    else:
                        raise Exception(f'Sample not ready for import: {sample.id} is missing the relationship for {relationship_key}')
            ...
            def _does_object_exist(self, attribute: any, identifier: str):
                """
                Checks if the object all ready exists within the Benchling ecosystem or is all ready loaded into memory
                ...
                """
                stored_values = factory.BENCHLING_OBJECT_MAP[factory.destination_object_type]['stored_values']
                converted_value_ids = factory.BENCHLING_OBJECT_MAP[factory.destination_object_type]['converted_value_identifiers']
                ...
                try:
                    benchling_obj = next(iter(factory.benchling.get_list(factory.destination_object_type, filter_obj)))
            ...
            def _sanitize_attribute(self, key: str, value: any, object_type_override:str = ''):
                """
                    This method sanitises an attribute making sure its the correct type expected by Benchling,
                    it will also transform the value of the attribute to a predetermined safe value for Benchling this is configured in VALUE_REPLACEMENTS.
                    ...
                    Args:
                         ...
                         object_type_override – This argument specifies the cleanup actions to be performed if the attribute does not belong to the destination object of the converter.
                ...
                if '' == object_type_override:
                    ...
                    if benchling_type and object_type_override in factory.benchling.schemas[benchling_type]:
